// ============================================================
//  SerThrocken (The Looking Glass 3D) — Rust FFmpeg Native Module
...
pub fn get_encoder_args() -> Vec<String> {
    if crate::gpu::get_gpu_info().vendor == "nvidia" {
        vec![
...
pub async fn apply_transition_native(
    ...
    let mut args_vec = vec!["-vf".into(), filter];
    args_vec.extend(get_encoder_args());
    args_vec.push("-c:a".into());
    args_vec.push("copy".into());
    run_ffmpeg_command(input, output, &args_vec).await
}
...
pub async fn apply_color_grade_native(
    ...
    let mut args_vec = vec!["-vf".into(), filter.to_string()];
    args_vec.extend(get_encoder_args());
    args_vec.push("-c:a".into());
    args_vec.push("copy".into());
    run_ffmpeg_command(input, output, &args_vec).await
